...
parser.add_argument('--run_name', type=str, default='new rescell',
...
def train_epoch(model, dataloader, optimizer, device, scaler, epoch, writer, args):
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        # Print tensor shape for debugging (no redimensioning)
...
def validate_memory_efficient(model, dataloader, device, beta_byte, beta_segment, writer, epoch, args, max_val_batches=50):
    """Memory-efficient validation function without redimensioning"""
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Print tensor shape for debugging (no redimensioning)
            if batch_idx == 0:
                print(f"Validation tensor shape: {x.shape}")
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, float(args.epochs - args.warmup_epochs - 1))
    ...
    for epoch in range(args.epochs):
        ...
        val_losses = validate_memory_efficient(
            model, val_dataset, device, beta_byte, beta_segment, writer, epoch, args
        )
